...
{
	int i;
	struct libinput *li[MAXDEVICES];
	struct libinput_event *event[MAXDEVICES];
	struct libinput_device *d[MAXDEVICES];
	int selectresult;
	fd_set fdset;
    ...
    if ((li[i] = libinput_path_create_context(&interface, NULL)) == NULL)
      die("Failed to initialize context");

    if ((d[i] = libinput_path_add_device(li[i], configs[i].device)) == NULL) {
  ...
  for (i = 0; i < MAXDEVICES; ++i) {
    FD_SET(libinput_get_fd(li[i]), &fdset);
  }
	...
	for (;;) {
        ...
        while ((event = libinput_get_event(li[i])) != NULL) {
          switch(libinput_event_get_type(event[i])) {
            case LIBINPUT_EVENT_TOUCH_DOWN: touchdown(event[i]); break;
            case LIBINPUT_EVENT_TOUCH_UP: touchup(event[i]); break;
            case LIBINPUT_EVENT_TOUCH_MOTION: touchmotion(event[i]); break;
          }
          libinput_event_destroy(event[i]);
			}
      }
		}
	}
...
main(int argc, char *argv[])
{
	int i, j;
	char *gestpt;
  char* device = configs[0].device;
  char* gestures = configs[0].gestures;
